      ...
      <div className="modal md style-editor">
        ...
        <div className="modal-body style-body">
          <div className="style-grid">
            <label className="dl-field"><span>{tr("字体")}</span><input className="text-input" value={v.font.name} onChange={(e) => patch({ font: { ...v.font, name: e.target.value } })} /></label>
            <label className="dl-field"><span>{tr("字号")}</span><input type="number" className="text-input" value={v.font.size} onChange={(e) => patch({ font: { ...v.font, size: Number(e.target.value) } })} /></label>
            <label className="dl-field"><span>{tr("描边")}</span><input type="number" className="text-input" step="0.5" value={v.border.border} onChange={(e) => patch({ border: { ...v.border, border: Number(e.target.value) } })} /></label>
            <label className="dl-field"><span>{tr("阴影")}</span><input type="number" className="text-input" step="0.5" value={v.border.shadow} onChange={(e) => patch({ border: { ...v.border, shadow: Number(e.target.value) } })} /></label>
          </div>
          {v.font && (
          ...
          {kind === "danmaku" && v.advanced && (
            <div className="style-grid">
              ...
              <label className="dl-field"><span>{tr("最小间距")}</span><input type="number" className="text-input" value={v.advanced.minimumGap} onChange={(e) => patch({ advanced: { ...(v.advanced as NonNullable<typeof v.advanced>), minimumGap: Number(e.target.value) } })} /></label>
          ...
          {kind === "subtitle" && v.color && (
            <div className="style-grid">
              <label className="dl-field"><span>{tr("主色")}</span><input className="text-input" value={v.color.primary} onChange={(e) => patch({ color: { ...v.color!, primary: e.target.value } })} /></label>
              <label className="dl-field"><span>{tr("次色")}</span><input className="text-input" value={v.color.secondary} onChange={(e) => patch({ color: { ...v.color!, secondary: e.target.value } })} /></label>
              <label className="dl-field"><span>{tr("描边色")}</span><input className="text-input" value={v.color.border} onChange={(e) => patch({ color: { ...v.color!, border: e.target.value } })} /></label>
              <label className="dl-field"><span>{tr("阴影色")}</span><input className="text-input" value={v.color.shadow} onChange={(e) => patch({ color: { ...v.color!, shadow: e.target.value } })} /></label>
              <label className="dl-field"><span>{tr("左距")}</span><input type="number" className="text-input" value={v.margin?.left ?? 10} onChange={(e) => patch({ margin: { left: Number(e.target.value), right: v.margin?.right ?? 10, vertical: v.margin?.vertical ?? 20 } })} /></label>
              <label className="dl-field"><span>{tr("右距")}</span><input type="number" className="text-input" value={v.margin?.right ?? 10} onChange={(e) => patch({ margin: { left: v.margin?.left ?? 10, right: Number(e.target.value), vertical: v.margin?.vertical ?? 20 } })} /></label>
              <label className="dl-field"><span>{tr("垂直距")}</span><input type="number" className="text-input" value={v.margin?.vertical ?? 20} onChange={(e) => patch({ margin: { left: v.margin?.left ?? 10, right: v.margin?.right ?? 10, vertical: Number(e.target.value) } })} /></label>
              <label className="dl-field"><span>{tr("对齐")}</span><select className="text-input" value={v.alignment ?? 2} onChange={(e) => patch({ alignment: Number(e.target.value) })}><option value={1}>{tr("左上")}</option><option value={2}>{tr("底部居中")}</option><option value={9}>{tr("右下")}</option></select></label>
            </div>
          )}
        </div>
