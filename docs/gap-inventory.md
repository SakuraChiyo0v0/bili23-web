...
| 登录态展示（SESSDATA） | 顶栏显示「已登录 xxx / 匿名模式」；无登录 UI 页（设计决策：不做登录，保留 SESSDATA） |
...
| **I-3 登录（扫码/短信）** | 仅 SESSDATA cookie | 设计决策：不做登录页，保留 SESSDATA。保持现状 |
| **I-4 代理三种模式** | config 只有 `advanced.proxy`（URL 字符串）；无「不用/系统/手动」三选一 | 原版有三种代理模式；当前仅手动 URL。需确认容器场景是否需要 |
| **I-5 高级默认画质/音质/编码未真正生效** | `applyRuntimeConfig` 未读取 `advanced.default*`；`createTasks` 未用默认档位兜底 | 用户在设置页填了默认画质，**实际创建任务不生效**，需接入取流 |
...
已提交并推送（`32f7b7a`）：
1. **已完成 P2-1 字幕语言选择**：下载面板 + 全局设置新增「指定语言」多选（zh/en/ja/ko/AI 等），后端已支持 `SubtitleLanguageSelection`，前端补齐控件与类型。
2. **已完成 P2-3 封面「附进后删除源图片」 + P2-2 部分（内嵌后删除源文件）**：弹幕/字幕「内嵌后删除」、封面「附进后删除」开关，与 `config.deleteAfter*` 对齐。
3. **已完成 P2-4 高级默认档位真正生效**：任务未显式指定画质/编码/音质时，用 `advanced.default*` 兜底（此前设置能保存但不生效）。已用真实视频验证：`defaultVideoQualityId=80` → 产物 1080P。
4. **追加修复空 style 崩溃**：前端未自定义样式会下发空 style，而引擎 ASS 渲染需要完整样式，新增 `normalizeExtrasStyles` 兜底回填默认样式（已用真实弹幕+字幕下载验证）。

仍待办：
- **P2-2 完整样式编辑面板**（字体/字号/颜色/边框/显示区/速度）：引擎与 config 支持，但 UI 尚未暴露样式字段编辑，仅做了空 style 兜底与删除开关。
- **P2-5 批处理「仅当前页 / 搜索全部」**：当前前端已暴露起始页/翻页数 + 关键词，但「仅当前页 vs 搜索全部」切换未做。
- **P3+ 筛选/排序参数**（I-1，原版本身未做，低优先；需 B 站接口确认可选值）。

> 说明：I-3（登录）、I-7（剪贴板/托盘）为设计层已明确不做，不在 P2 范围。I-4（代理三模式）先与用户确认是否需要。
