...
1. **P2-1 字幕语言选择**（后端已就绪，前端补控件即可，收益高）
2. **P2-2 弹幕/字幕样式编辑面板**（补「内嵌后删除」「指定语言」「样式表」）
3. **P2-3 封面「嵌入后删除原图」开关**（纯前端）
4. **P2-4 高级默认档位真正接入**（I-5，需后端把 `advanced.default*` 作为任务默认档位兜底）
5. **P2-5 批处理「仅当前页 / 搜索全部」**（I-2，需后端或前端分页协作）
6. **P3+ 筛选/排序参数**（I-1，原版本身未做，低优先；需 B 站接口确认可选值）
