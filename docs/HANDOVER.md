...
> 最近更新：2026-09-05（main 已重建 React/Vite 前端并由同一服务静态托管 SPA，本文“纯后端”句子已作为历史说明）。
...
- 解析成功后工具条提供 **全选 / 反选 / 全不选 / 展开 / 收起 / 清空**；支持快捷键 Ctrl+A 全选、Ctrl+D 全不选（输入框内不触发）。
- 解析树分组行点击 = 整组勾选/取消；行首箭头仅负责展开/折叠，互不干扰。
- 移动端（≤640px）：工具条改换行展示，不再横向滚动隐藏按钮；树保留“勾选 + 标题”两列。
- **下载目录“浏览…”**：设置页可打开 NAS 目录树选择器（`GET /api/dirs`）逐层点选容器内可读目录并回填 `download.dir`；
  浏览器无法访问 NAS 容器所在机器的本地目录，因此不用桌面版原生“选文件夹”，避免选到无效路径。

- **解析历史**：侧栏/移动端顶栏均有“解析历史”入口，可查看已解析链接、一键重新解析（跳回解析页）或删除单条；
  数据来自 `GET /api/parse-history`（后端早已提供）。
