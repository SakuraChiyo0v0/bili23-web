...
> 最近更新：2026-09-05（前端已完成原版功能逐项补齐：登录账号卡/真实头像、解析批量选择与批量解析、站内服务端搜索、Shift 范围勾选、右键菜单、列表设置、队列排序、下载通知、目录树选择等；详见 §5“前端交互与登录”）。
...
- **当前阶段**：`main` 为**Hono REST/SSE 后端 + React/Vite 前端**。前端源码在 `apps/web/src/client`，构建产物 `dist/client` 由同一服务静态托管。
  曾经移除的旧 React/Vite 前端快照在 `backup/frontend-20260904`，已确认废弃，不作为当前前端基准。
...
> 注：前端依赖（React/Vite/@vitejs/plugin-react/qrcode）在 `main` 的 `apps/web/package.json` 中；构建输出 `apps/web/dist/client`。
...
packages/engine/   下载引擎（纯库，无 HTTP 服务）
  ...
  src/index.ts     引擎统一导出
  tests/           引擎单元测试（vitest）
apps/web/          Web 端：Hono 后端 + React/Vite 前端（src/client → dist/client 静态托管）
...
> `main` 上已有 `apps/web/src/client`（React 前端）、`index.html`、`vite.config.ts`、`tsconfig.client.json`与前端测试。
> 服务启动后 `/` 与 `/assets/*` 托管前端 SPA；`/api/*` 仍为后端接口。
...
对齐原版三页签：
- **媒体设置**：画质/音质/编码下拉 + 时长与预计大小（按选中或最高可用码率估算） + 视频/音频/合并/保留原文件开关。
- **附加文件**：弹幕（格式 XML/ASS/JSON、样式、嵌入、嵌入后删除）、字幕（格式 SRT/LRC/TXT/ASS/JSON、语言选择、样式、嵌入、嵌入后删除）、封面（格式 JPG/PNG/AVIF/WEBP、嵌入，AVIF 不可嵌入）、章节、元数据（NFO/JSON）。
- **下载设置**：输出容器 MP4/MKV、本次命名规则选择（默认沿用全局）。
- 确定后作为任务快照固化，不写回全局默认（全局默认在设置页管理）。

### 前端交互与登录（2026-09 逐项对齐原版后）

**登录与账号**
- 登录态来自 B 站 `nav` 接口，展示真实头像/昵称/UID（`/api/auth/status` 返回 `uname/face/mid`，持久化于 `auth.json`）。
- 点击头像/账号弹出账号卡：头像/昵称/UID + “打开 B 站主页” + “退出登录”（对齐原版 ProfileCard）。移动端顶栏也有账号/登录入口。

**解析页**
- 工具条：**全选 / 反选 / 全不选 / 展开 / 收起 / 清空 / 批量选择 / 批量解析 / 站内搜索…**；Ctrl+A 全选、Ctrl+D 全不选（输入框内不触发）。
- **批量选择**：按行号 `1,3,5-10` 勾选（分P/合集按整组）。
- **批量解析**：多行链接逐条解析、单条失败不阻塞，可选“解析后自动全选”。
- **搜索**：本地搜索框支持“全选匹配/清除匹配”；space/favlist/history/watch_later 提供站内服务端搜索（收藏夹 keyword 由后端拼 URL）。
- **Shift 范围勾选**：按住 Shift 点选另一行，勾选两者之间连续范围。
- **右键菜单**（解析行）：叶子=仅勾选此项/下载此项/复制链接；分组=仅勾选组/全选取消整组/展开收起。
- 分组行点击整组勾选；行首箭头仅负责展开/折叠。
- 移动端（≤640px）：工具条换行展示；树保留“勾选 + 标题”两列（可设置恢复副信息列）。

**设置页（解析与行为组）**
- 真实生效开关：显示标签/时长/时间列（关闭后两列紧凑视图）、交替行色（偏好存 `localStorage`）。
- 下载完成/失败通知开关（toast + 浏览器通知，开启时请求授权）。

**下载页**
- 下载队列排序偏好：字段（创建时间/标题/进度/大小/状态）+ 升降序（存 `localStorage`）。
- 任务终态（完成/失败/取消）toast；开启浏览器通知后同步系统通知（首次进入页面只建基线，不提示历史任务）。

**下载选项弹窗**
- 媒体信息显示 DASH/单文件 MP4 类型徽标；画质/音质下拉展示码率。
- 附加文件与下载设置完整选项（见上文“下载选项弹窗”小节）。

**下载目录“浏览…”**
- 设置页打开 NAS 目录树选择器（`GET /api/dirs`）逐层点选容器内可读目录并回填 `download.dir`；
  浏览器无法访问 NAS 容器所在机器的本地目录，因此不用桌面版原生“选文件夹”，避免选到无效路径。

**解析历史**
- 侧栏/移动端顶栏均有“解析历史”入口，可查看已解析链接、一键重新解析（跳回解析页）或删除单条；
  数据来自 `GET /api/parse-history`。
...
> 镜像变化影响：push main 会自动重建镜像并触发 watchtower。当前镜像包含前端（vite build 后静态托管），
> 重建后浏览器访问 `:8788` 会看到前端页面（若 `dist/client` 未构建则 `/` 404）。
...
2. **编辑前端需先 `pnpm --filter @bili23-web/web build`**：启动服务时若 `dist/client` 不存在，`/` 会返回 404。开发时可用 `dev:client`（Vite dev server）。
...
### 必须
- [x] **重建前端**：2026-09-05 已在 `main` 完成 React/Vite 前端（解析/下载/设置/登录/收藏夹），构建后由同一服务静态托管。
      前端源码 `apps/web/src/client`，对接 `/api/*`；开发用 `pnpm --filter @bili23-web/web dev:client`。


### 建议
- [x] 解析翻页/聚合：space/favlist/history/watch_later 支持设置“翻页数”，服务端搜索会按 keyword 自动跨页聚合（前端不做逐页手动翻页）。
- [ ] 解析列表“悬浮命令条”（右键菜单已覆盖核心操作，未做持续悬浮条样式）。
